...
# def train_options_finetune(base_args):
#     # base_args = base_parser().parse_known_args()[0]
...
def train_options_finetune(opt_dict):
    # 1. 把 YAML 字典伪装成命令行参数列表
    mock_args = []
    for key, value in opt_dict.items():
        if isinstance(value, bool):
            # 如果是布尔值且为 True，通常命令行里只写 --flag_name (比如 --save_results)
            if value:
                mock_args.append(f"--{key}")
        elif isinstance(value, list):
            # 如果是列表 (比如 --heads 4 8 16)
            mock_args.append(f"--{key}")
            mock_args.extend([str(v) for v in value])
        else:
            # 常规键值对 (比如 --model MoCE_IR_S)
            mock_args.extend([f"--{key}", str(value)])
            
    # 2. 显式地把 mock_args 传给 parser，这样它就不会去读终端输入了
    base_parser_instance = base_parser()
    base_args = base_parser_instance.parse_known_args(mock_args)[0]
    
    print(base_args)  # 打印解析后的基础参数，确认它们被正确解析了
    if base_args.model == "MoCE_IR_S":
        parser = moce_ir_s(base_parser())
    elif base_args.model == "MoCE_IR":
        parser = moce_ir(base_parser())
    ...
    # 3. 同样显式地传入 mock_args
    options = parser.parse_args(mock_args)

    # 如果使用了梯度累加，调整 batch_size
    if options.accum_grad > 1:
        options.batch_size = options.batch_size // options.accum_grad
